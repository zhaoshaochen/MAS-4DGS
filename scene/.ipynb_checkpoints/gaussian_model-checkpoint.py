...
import random  # 添加random用于调试输出控制


# 添加MLP网络类定义
# 添加MLP网络类定义（修改版）
class MotionStatePredictor(nn.Module):
    """MLP网络用于预测每个高斯球在特定时刻的运动状态(0或1)"""

    def __init__(self, hidden_dim=512, num_layers=5, time_dim=128, dropout=0.1):
        """
        Args:
            hidden_dim: 隐藏层维度，从256增加到512
            num_layers: 网络层数，从3增加到5
            time_dim: 时间编码维度，从64增加到128
            dropout: Dropout概率，新增参数
        """
        super().__init__()
        ...
        # 主网络层 - 增加深度和Dropout
        layers = [nn.ReLU(), nn.Dropout(dropout)]  # 第一层后添加Dropout
        
        for i in range(num_layers - 2):
            layers.append(nn.Linear(hidden_dim, hidden_dim))
            layers.append(nn.ReLU())
            # 在每个隐藏层后添加Dropout（除了最后一层）
            if i < num_layers - 3:
                layers.append(nn.Dropout(dropout))

        self.feature_layers = nn.Sequential(*layers)
        self.output_layer = None  # 将在初始化高斯球后设置

    def fourier_encode_time(self, t, max_freq=15):
        """
        对时间进行傅里叶位置编码
        Args:
            t: shape [batch_size, 1], 值在[0, 1]之间
            max_freq: 最大频率，从10增加到15以捕捉更精细的时间模式
        """
        # 生成频率序列
        ...
        ], dim=-1).squeeze(1)  # [batch_size, 2*max_freq]

        # 填充或截断到time_dim维度
        if encoded.shape[1] < self.time_dim:
            padding = torch.zeros(encoded.shape[0], self.time_dim - encoded.shape[1], device=t.device)
            encoded = torch.cat([encoded, padding], dim=1)
        else:
            encoded = encoded[:, :self.time_dim]

        return encoded

    def update_output_dim(self, num_gaussians, copy_weights=None, parent_indices=None):
        """
        更新输出层以匹配高斯球数量，支持权重继承
        
        Args:
            num_gaussians: 新的高斯点数量
            copy_weights: 权重复制策略 ("expand" 或 tensor)
            parent_indices: 新点对应的父点索引，用于权重继承
        """
        # 获取隐藏层维度（注意：由于添加了Dropout，需要找到最后一个Linear层）
        hidden_dim = None
        ...
        if hidden_dim is None:
            hidden_dim = 512  # 默认值（与__init__中的hidden_dim一致）
            ...
            with torch.no_grad():
                if copy_weights == "expand":
                    ...
                    if selected_weights.shape[0] == num_gaussians:
                        ...
                        print(f"Warning: Weight dimension mismatch. Expected {num_gaussians}, got {selected_weights.shape[0]}")
        else:
            # 首次初始化时，设置负偏置使新点默认为"静止"
            with torch.no_grad():
                self.output_layer.bias.data[:] = -2.0

    def forward(self, t, normalize_time=True, time_range=(0, 1)):
        """
        输入: t - 时间标量或批次时间 [batch_size, 1]
        输出: 每个高斯球的0/1状态概率 [batch_size, num_gaussians]
        """
        if t.dim() == 0:
        ...
        # 傅里叶编码（使用更高的max_freq=15）
        t_encoded = self.fourier_encode_time(t, max_freq=15)

        # 通过网络
        features = self.time_encoder(t_encoded)
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, args):
        ...
        self.motion_threshold = 0.1922  # 运动量阈值，可调整
        self.last_deformation_output = None  # 缓存最近的变形输出
        self.time_range = None  # 时间范围，用于归一化

        # 添加用于缓存预测结果的属性
        self.cached_motion_states = None  # 缓存的运动状态预测结果
        # ===== 添加全量变形标志 =====
        self.use_full_deformation = False  # 默认使用选择性变形
        # ===== 标志初始化结束 =====

    def predict_motion_states(self, time):
        """使用训练好的MLP预测每个高斯球在时刻t的运动状态"""
        if self.motion_predictor is None or self.motion_predictor.output_layer is None:
            return torch.ones((self._xyz.shape[0],), device="cuda")  # 如果没有MLP，全部返回1
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int):
        ...
        # 设置时间范围（假设time_line是总帧数）
        self.time_range = (0, time_line - 1) if time_line > 0 else (0, 1)

    def compute_motion_labels(self, time):
        """根据缓存的变形输出计算运动标签，添加调试信息"""
        if self.last_deformation_output is None:
            return torch.zeros((self._xyz.shape[0],), device="cuda")

        # last_deformation_output: (means3D, scales, rotations, opacity)
        deformed_xyz = self.last_deformation_output[0]

        # 计算运动量（相对于canonical的位移）
        motion_magnitude = torch.norm(deformed_xyz - self._xyz, dim=1)

        # 添加调试输出（以较低频率输出，避免刷屏）
        # if random.random() < 0.05:  # 看阈值选这一段
        #     with torch.no_grad():
        #         mean_mag = motion_magnitude.mean().item()
        #         median_mag = motion_magnitude.median().item()
        #         max_mag = motion_magnitude.max().item()
            
        #         # 计算不同百分位点的值，这对于设定阈值至关重要
        #         p25 = torch.quantile(motion_magnitude, 0.25).item()
        #         p75 = torch.quantile(motion_magnitude, 0.75).item()
        #         p90 = torch.quantile(motion_magnitude, 0.90).item()

        #         print("\n" + "="*50)
        #         print(f"[Motion Stats Debug] Iteration Info:")
        #         print(f"  - Mean: {mean_mag:.4f}, Median (p50): {median_mag:.4f}, Max: {max_mag:.4f}")
        #         print(f"  - Percentiles: p25={p25:.4f}, p75={p75:.4f}, p90={p90:.4f}")
        #         print(f"  - Current Threshold: {self.motion_threshold}")
            
        #         # 根据当前阈值计算的活跃比例
        #         active_ratio = (motion_magnitude > self.motion_threshold).float().mean().item()
        #         print(f"  - Active Ratio with current threshold: {active_ratio:.3f}")
        #         print("="*50 + "\n")

        if random.random() < 0.01:  # 训练时用这一段
            mean_magnitude = motion_magnitude.mean().item()
            max_magnitude = motion_magnitude.max().item()
            median_magnitude = motion_magnitude.median().item()
            
            
            motion_labels_temp = (motion_magnitude > self.motion_threshold).float()
            active_label_ratio = motion_labels_temp.mean().item()
            
            print(f"[Motion Debug] Motion magnitude stats: mean={mean_magnitude:.4f}, "
                  f"max={max_magnitude:.4f}, median={median_magnitude:.4f}")
            print(f"[Motion Debug] Current active label ratio: {active_label_ratio:.3f} "
                  f"with threshold {self.motion_threshold}")
        
        # 根据阈值生成0/1标签
        motion_labels = (motion_magnitude > self.motion_threshold).float()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
        ...
        Args:
            parent_indices: 新点对应的父点索引，用于MLP权重继承
        """
        d = {"xyz": new_xyz,
        ...
        # 更新MLP输出维度，传递parent_indices用于权重继承
        if self.motion_predictor is not None:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # 生成parent_indices：每个被分裂的父点索引重复N次
        selected_indices = torch.where(selected_pts_mask)[0]  # 获取被选中父点的索引
        parent_indices_for_new = selected_indices.repeat_interleave(N)  # 每个父点索引重复N次
        ...
        # 调用densification_postfix，传递parent_indices
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20,
        ...
        # 生成parent_indices：克隆的点对应于原始点的索引
